...
import { deleteNoteAction, updateNoteMetaAction } from "./_actions";
...
export function NoteActions({
  ...
  function save() {
    ...
    startTransition(async () => {
      const result = await updateNoteMetaAction({
        id,
        childId: editChild,
        date: editDate,
      });
  ...
  return (
    ...
    >
      <button
        ...
        className="grid h-8 w-8 place-items-center rounded-md bg-white/70 text-slate-400 backdrop-blur hover:bg-slate-100 hover:text-slate-700"
        aria-label="더보기"
      >
      ...
      {open === "edit" && (
        <Modal title="알림장 편집" onClose={close}>
      ...
      {open === "delete" && (
        <Modal title="알림장 삭제" onClose={close}>
          <p className="text-sm text-slate-700">
            이 알림장을 삭제할까요?{" "}
            <strong className="text-rose-600">되돌릴 수 없습니다.</strong>
          </p>
          {error && (
